...
import job.devices.SSD1306;
import job.devices.lcdutils.ScreenBuffer;
import job.devices.lcdutils.img.ImgInterface;
import job.devices.lcdutils.img.Java32x32;
...
public class SSD1306Sample {
	...
	private final static boolean NORMAL_MODE = true;
	private final static int TOP_LEFT_X_OFFSET = 0;
    ...
    public static void main(String... args) {
        ...
        if (NORMAL_MODE || DEBUG_MODE) {
            // Marquee
            if ("true".equals(System.getProperty("verbose", "false"))) {
                System.out.println("Marquee...");
            }
            for (int i = 0; i < 128; i++) {
                sb.image(img, TOP_LEFT_X_OFFSET - i, 0);
...
//      System.out.println("Displaying " + ca[c] + " at " + x + ", " + y + ", i=" + i + ", strOffset=" + strOffset);
                    sb.text(String.valueOf(ca[c]), xpos, ypos);
                }
                oled.setBuffer(mirror ? SSD1306.mirror(sb.getScreenBuffer(), WIDTH, HEIGHT) : sb.getScreenBuffer());
                oled.display();
//      delay(75);
