...
public class ScreenBuffer {
	...
	public void clear(Mode mode) {
		...
		for (int i = 0; i < this.screenBuffer.length; i++) {
			this.screenBuffer[i] = (byte)(mode == Mode.WHITE_ON_BLACK ? 0 : 1);
		}
	}
...
//			println(bmVal + ", 0x" + lpad(Integer.toHexString(bmVal), 2, "0") + ", 0b" + lpad(Integer.toBinaryString(bmVal), 8, "0"));
				this.screenBuffer[(this.w * (line)) + col] = (byte)(bmVal & 0xFF);
			}
		}
