...
static class Simulators {
  ...
  public static class HumSimulator {
    ...
    public static float get() {
      ...
      return (float)lastHum;
    }
  }

  /**
   * Pressure simulator, in hPa
   */
  public static class PressSimulator {
    private final static int MINI =  950;
    private final static int MAXI = 1050;
    private static double lastPress = 1013.60d; // Start here
    
    public static float get() {
      int sign = (int)System.currentTimeMillis() % 2;
      double diff = Math.random() * (sign == 0 ? 1 : -1);
      lastPress += diff;
      lastPress = Math.max(lastPress, MINI);
      lastPress = Math.min(lastPress, MAXI);
      return (float)lastPress;
    }
  }
}
